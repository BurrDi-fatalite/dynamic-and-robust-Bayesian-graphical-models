...
mkdir('Simulation_results') % save MCMC output from the simulation study
mkdir('Simulation_analysis') % save model performance results
...
% Examples of how to run the proposed models
...
% Save results for s = 2 and s = 4
save(strcat('Simulation_results/sim_results_',data_method,'_',model_method,'4states_rep_', num2str(rep)), 'results')
save(strcat('Simulation_results/sim_results_',data_method,'_',model_method,'4states_states_rep_', num2str(rep)), 'states_save')
...
open('Helper_functions/Simulation_25rep.m')
% Joint graphical lasso model, R code, parallel computing:
open('Helper_functions/Simulation_JGL_25rep.R')

%% Analyze results of 25 replications and generate plots
% Generate Table 1, Table 2, Figure 3 and Figure 4 in paper
open('Helper_functions/Simulation_analyze_results_25rep.m')

%%%%%%%%%%%%%%%%%%%%%%
%% Run simulation on the same 25 data sets with different number of hidden states S
clear
open("Helper_functions/Simulation_varyingS.m")
open("Helper_functions/Simulation_analyze_results_varyingS.m")  % compute WAIC of each replication to choose S

%% Run Simulation study with p = 10
open('Helper_functions/Simulation_study_appendix.m')
